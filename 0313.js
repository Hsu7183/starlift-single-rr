// 0313.js — 以「次新檔」為基準合併後，直接餵給 single-trades.js（自動計算）
// 分析資料 = 基準全段 + 最新檔的新增；畫面只顯示期間＋KPI＋資產曲線
// ★已移除 manifests/0313.json 的任何讀取/寫入，以消除 DevTools 400 紅字
...
(function () {
  ...
  async function boot() {
    try {
      ...
      if (paramFile) {
        latest = {
          name    : paramFile.split('/').pop() || '0313.txt',
          fullPath: paramFile,
          from    : 'url'
        };
      } else {
        ...
        if (!list.length) {
          setStatus('找不到檔名含「0313」的 TXT（可用 ?file= 指定）。', true);
          return;
        }
      ...
      if (!latest) {
        setStatus('找不到可分析的 0313 檔案。', true);
        return;
      }
