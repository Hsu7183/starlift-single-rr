...
(function (global) {
  ...
  function mapAct(act) {
    ...
    // 明確排除：平買 / 平賣 / 強平 / 強制平倉 / 其他
    return 'OTHER';
  }

  /* ======================== TXT 解析 ======================== */
  function parseEtfTxt(rawTxt){
    // 全域淨化：換行、BOM、零寬、NBSP、全形空白
    var norm = String(rawTxt||'')
      .replace(/\r\n?/g, '\n')
      .replace(/[\uFEFF\u200B-\u200D]/g,'');
    ...
    for (var i=0;i<lines.length;i++){
      var L = (lines[i]||'').replace(/[\u00A0\u3000]/g,' ').trim(); // NBSP/全形空白→半形空白
      if (!L) continue;
      if (/^日期\s*,\s*時間\s*,\s*價格\s*,\s*動作/i.test(L)) continue; // header

      // 允許半形/全形逗號；動作中文，尾端任意 key=value
      ...
      // tail：key=value 以全/半形逗號分割；允許中文 key
      var kv = {};
    ...
    for (var u=0; u<tids.length; u++){
      ...
      // 第一個 BUY/ADD1/ADD2 視為起點
      var firstEv = null;
      for (var f=0; f<arr.length; f++){
        if(arr[f].action==='BUY' || arr[f].action==='ADD1' || arr[f].action==='ADD2'){ firstEv = arr[f]; break; }
      }
      if(!firstEv) continue;
      ...
      trades.push({
        ...
        qty:qty,
        pxIn:pxInPrint, pxOut:pxOutPrint, pxInAvg:pxInAvg,
        gainSlip:gainNet,
  ...
  function calcEtfKpis(parsed){
    ...
    var avgPart=mean(parts), p75Part=parts.length? quantile(parts.map(function(x){return x*100;}),0.75)/100 : 0;
