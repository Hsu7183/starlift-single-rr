// 股票｜雲端單檔分析（KPI49，0807 表格樣式，ES5 相容）
(function(){
  'use strict';
  ...
  function listCloud(){
    ...
    var p=(prefix.value||'').trim(); var fixed = p && p.charAt(p.length-1)!=='/' ? (p+'/') : p;
  ...
  function previewCloud(){
    ...
    getUrl(path).then(function(u){ if(!u){ prev.textContent='取得連結失敗'; return null; } return fetch(u,{cache:'no-store'}); })
      .then(function(r){ if(!r){return null;} if(!r.ok){ prev.textContent='HTTP '+r.status; return null; } return r.arrayBuffer(); })
      .then(function(ab){ if(!ab) return; var best=decodeBest(ab); meta.textContent='來源：'+path+'（編碼：'+best.enc+'）';
  ...
  function importCloud(){
    ...
    getUrl(path).then(function(u){ if(!u){ alert('取得連結失敗'); return null; } return fetch(u,{cache:'no-store'}); })
      .then(function(r){ if(!r){return null;} if(!r.ok){ alert('HTTP '+r.status); return null; } return r.arrayBuffer(); })
      .then(function(ab){ if(!ab) return; var best=decodeBest(ab); runAll(best.txt); });
  }
  ...
  function normalize(txt){
    return (txt||'').replace(/\ufeff/gi,'').replace(/[\u200B-\u200D\uFEFF]/g,'').replace(/[\x00-\x09\x0B-\x1F\x7F]/g,'')
      .replace(/\r\n?/g,'\n').split('\n').map(function(s){ return s.replace(/\s+/g,' ').trim(); }).filter(function(x){ return !!x; });
  }
  function toCanon(lines){
    var out=[], i, l, m, d8, t6, px, act;
    for(i=0;i<lines.length;i++){
      l=lines[i]; m=l.match(CANON_RE);
      if(m){ out.push({ts:m[1], px:+m[2], act:m[3]}); continue; }
      m=l.match(CSV_RE);
      if(m){ d8=m[1]; t6=pad6(m[2]); px=+m[3]; act=mapAct(m[4]); if(isFinite(px) && /^\d{6}$/.test(t6)) out.push({ts:d8+t6, px:px, act:act}); }
    }
    out.sort(function(a,b){ return a.ts.localeCompare(b.ts); }); return out;
  }
  ...
  function backtest(rows){
    ...
    for(var i=0;i<rows.length;i++){
      ...
      if(r.act==='新買'){
        ...
        if(cash>=amt+f){ cash-=(amt+f); shares+=CFG.unit; cumCost+=(amt+f);
        ...
        var day=d8(r.ts); dayPnL.set(day,(dayPnL.get(day)||0)+pnl);
  ...
  function timeUnderwater(eq){
    ...
    for(i=0;i<eq.length;i++){ v=eq[i]; if(v>=peak){ peak=v; if(cur>0){ totalTU+=cur; cur=0; } } else { cur++; if(cur>maxTU) maxTU=cur; } }
    if(cur>0) totalTU+=cur; return { maxTU:maxTU, totalTU:totalTU };
  }
  ...
  function computeKPI(bt){
    ...
    var mdd=maxDrawdown(eq).mdd, ui=ulcerIndex(eq), sr=sharpe(annRet,annVol,CFG.rf), so=sortino(annRet,dStd,CFG.rf);
    ...
    return {
      ...
      // Distribution (8)
      tradeCount:nTrades,posCount:tradePnl.filter(function(x){return x>0;}).length,zeroCount:tradePnl.filter(function(x){return x===0;}).length,negCount:tradePnl.filter(function(x){return x<0;}).length,
      posRatio:nTrades? tradePnl.filter(function(x){return x>0;}).length/nTrades:0,negRatio:nTrades? tradePnl.filter(function(x){return x<0;}).length/nTrades:0,omega:omg,pnlStd:statsBasic(tradePnl).std
    };
  }
  ...
  function renderWeeklyChart(weeks){
    ...
    var ctx=$('#chWeekly'); if(chWeekly) chWeekly.destroy();
    chWeekly=new Chart(ctx,{ data:{ labels:labels, datasets:[
  ...
  function bandTxt(score){
    ...
    return score===0?'Strong（強）':score===1?'Adequate（可接受）':'Improve（優化）';
  }
  function scoreMetric(key,v){
    // 簡化分級：可再按你實際口徑調整
    if(key==='annVol' || key==='dStd' || key==='mdd' || key==='costRatio'){ return v<=0.15?0:(v<=0.25?1:2); }
    if(key==='sr' || key==='so' || key==='cal' || key==='mar' || key==='pf'){ return v>=1.5?0:(v>=1.0?1:2); }
    if(key==='hitRate'){ return v>=0.55?0:(v>=0.45?1:2); }
    if(key==='totalReturn' || key==='annRet'){ return v>=0.10?0:(v>=0.05?1:2); }
    return 1;
  }
  function trHTML(name, val, key, ref){
    var sc=scoreMetric(key,val.raw); var cls=sc===2?'improve-row':(sc===0?'ok-row':'');
    return '<tr class="'+cls+'"><td>'+name+'</td><td>'+val.disp+'</td><td>'+(sc===2?'建議優化':'維持監控')+'</td><td>'+bandTxt(sc)+'</td><td>'+ref+'</td></tr>';
  }
  function theadHTML(){ return '<thead><tr><th>指標</th><th>數值</th><th>建議</th><th>機構評語</th><th>參考區間</th></tr></thead>'; }

  function renderKPI(k){
    // 建議優先（3 項示例：波動率、PF、最大虧損）
    var topTB=$('#kpiTop tbody');
    topTB.innerHTML = [
      trHTML('波動率（Volatility, ann.）', {disp:pct(k.annVol),raw:k.annVol}, 'annVol', '愈低愈好 ≤15%'),
    ...
    fillTable('#kpiDist', dist);
  }

  function fillTable(sel, rows){
    var el=$(sel); if(!el) return;
    el.innerHTML = theadHTML();
    var tb=document.createElement('tbody'); var html='';
    for(var i=0;i<rows.length;i++){ html+=trHTML(rows[i][0], rows[i][1], rows[i][2], rows[i][3]); }
  ...
  function renderTrades(t){
    ...
    for(i=0;i<t.length;i++){
      r=t[i];
      rows+='<tr class="'+(r.kind==='SELL'?'ok-row':'')+'"><td>'+r.ts+'</td><td>'+r.kind+'</td><td>'+r.px.toFixed(3)+'</td><td>'+r.shares+'</td>'
