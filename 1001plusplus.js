// 1001plusplus.js — 以「次新檔」為基準合併後，直接餵給 single-trades.js（自動計算）
// 分析資料 = 基準全段 + 最新檔的新增
// ★不讀 manifest，避免多餘 GET / 400 紅字
// ★若最新檔包含比基準更早的歷史段，會補在最前面
// ★輸出前做「配對清洗」，確保 canonical 行永遠是 開倉→平倉 成對
(function () {
  'use strict';
  ...
  async function feedToSingleTrades(filename, mergedText) {
    ...
    const fname = filename || '1001plusplus.txt';
  ...
  async function boot() {
    try {
      ...
      if (paramFile) {
        latest = {
          name    : paramFile.split('/').pop() || '1001plusplus.txt',
          fullPath: paramFile,
          from    : 'url'
        };
      } else {
      ...
      if (!latest) {
        setStatus('找不到可分析的 1001plus+ 檔案。', true);
        return;
      }
      ...
      if (btnBase) {
        ...
        btnBase.title = '此頁不寫入基準，基準固定為次新檔。';
        btnBase.onclick = null;
      }

      // 6) 加一行 header + canonical 3 欄餵給 single-trades.js
      const finalText = '1001PLUSPLUS MERGED\n' + mergedText;
