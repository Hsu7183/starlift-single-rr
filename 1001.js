// 1001.js — 與 0807 版同邏輯：基準全段 + 基準後新增；畫面只顯示期間文字
(function () {
  ...
  const WANT = /1001/i;
  const MANIFEST_PATH = "manifests/1001.json";
  ...
  // 傳給 single.js（避免它再猜編碼 & params.map 錯誤）
  ...
  async function feedToSingle(filename, decodedText){
    ...
    const file = new File([decodedText], filename || '1001.txt', { type:'text/plain' });
    const dt = new DataTransfer(); dt.items.add(file);
    input.files = dt.files;
    input.dispatchEvent(new Event('change', { bubbles:true }));
  }

  // manifest：下載（第一次不存在時可能 400，無礙）
  ...
  async function boot(){
    try{
      ...
      if (paramFile){
        latest = { name:paramFile.split('/').pop()||'1001.txt', fullPath:paramFile, from:'url' };
      } else {
      ...
      if(!latest){ set('找不到檔名含「1001」的 TXT（可用 ?file= 指定）。', true); return; }
      ...
      if (base){
        ...
        const m = mergeByBaseline(rBase.canon, rNew.canon); // 合併：基準全段 + 基準後新增
      ...
      // 4) 顯示期間（YYYYMMDD 開始到 YYYYMMDD 結束）
