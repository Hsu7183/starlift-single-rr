// etf-00909.js — 控制器（全費口徑 + 累計成本 + 平均報酬率 + 最佳化 1-1-2「先算最大總張數，再分攤」）
(function () {
  ...
  function renderOptTable(rows){
    ...
    tbody.innerHTML='';
    for(const r of rows){
      const tr=document.createElement('tr'); tr.className=(r.side==='BUY'?'buy-row':'sell-row');
      tr.innerHTML=`<td>${r.side==='BUY'?'買進':'賣出'}</td>
      <td>${tsPretty(r.ts)}</td><td>${r.price.toFixed(2)}</td>
      <td>${r.avgCost!=null? r.avgCost.toFixed(2):'—'}</td><td>${r.shares.toLocaleString()}</td>
      <td>${Math.round(r.buyAmount||0).toLocaleString()}</td>
      <td>${Math.round(r.sellAmount||0).toLocaleString()}</td>
      <td>${Math.round(r.fee||0).toLocaleString()}</td>
      <td>${Math.round(r.tax||0).toLocaleString()}</td>
      <td>${Math.round(r.cost||0).toLocaleString()}</td>
      <td>${Math.round(r.cumCost||0).toLocaleString()}</td>
      <td>${r.pnlFull==null?'—':pnlSpan(r.pnlFull)}</td>
      <td>${fmtPct(r.retPctUnit)}</td>
      <td>${r.cumPnlFull==null?'—':pnlSpan(r.cumPnlFull)}</td>`;
      tbody.appendChild(tr);
    }
  }
  ...
  // ===== 最佳化：先用第一筆價算「最大總張數」，再 1/1/2 分配；實單依當時價檢查剩餘資金 =====
  function buildOptimizedExecs(execs){
    const segs=splitSegments(execs); const out=[]; let cumPnlAll=0;

    for(const seg of segs){
      const buys=seg.filter(x=>x.side==='BUY'); const sell=seg.find(x=>x.side==='SELL');
      if(!buys.length || !sell) continue;

      // 1) 用第一筆買價算最大總張數（含買方手續費）
      const p0=buys[0].price;
      const oneLot=buyCostLots(p0,1).cost; if(oneLot<=0) continue;
      let maxLotsTotal=Math.floor(OPT.capital / oneLot);
      if(maxLotsTotal<=0) continue;

      // 2) 分配 1/1/2 → q = floor(maxLotsTotal / 4)
      let q=Math.floor(maxLotsTotal/4); if(q<=0) q=1; // 至少 1 張規劃
      const n=Math.min(3, buys.length);
      const plan=[q, q, 2*q].slice(0, n);

      // 3) 逐筆進場：用「當時價」與「剩餘資金」檢查，不足就下修
      let remaining=OPT.capital, sharesHeld=0, avgCost=0, cumCost=0, unitCount=plan.reduce((a,b)=>a+b,0);

      for(let i=0;i<n;i++){
        const b=buys[i];
        let lotsPlanned=plan[i];

        // 在當時價格下可買的最大 lots
        let affordable=Math.floor(remaining / buyCostLots(b.price,1).cost);
        if(affordable<=0) break;
        if(lotsPlanned>affordable) lotsPlanned=affordable;

        const bc=buyCostLots(b.price, lotsPlanned);
        ...
        out.push({
          ...
          avgCost:newAvg, shares:bc.shares,
          buyAmount:bc.f.gross, sellAmount:0,
          fee:bc.f.fee, tax:0, cost:bc.cost, cumCost,
      ...
      // 4) 賣出：在段尾一次出清
      if(sharesHeld>0){
        const s=sell; const sellGross=fees(s.price, sharesHeld, true);
        const pnlFull = sellGross.gross - cumCost - (sellGross.fee + sellGross.tax);
        ...
        out.push({
          ...
          buyAmount:0, sellAmount:sellGross.gross,
          fee:sellGross.fee, tax:sellGross.tax,
          cost:0, cumCost, pnlFull, retPctUnit, cumPnlFull:cumPnlAll
        });
      }
    }
    return out;
  ...
  async function boot(){
    try{
      ...
      // 設基準
      const btn=$('#btnSetBaseline'); btn.disabled=false; btn.onclick=async()=>{ try{ await writeManifest({ baseline_path: latest.from==='url'? latest.fullPath : latest.fullPath, updated_at:new Date().toISOString() }); btn.textContent='已設為基準'; }catch(e){ set('寫入基準失敗：'+(e.message||e), true); } };
      ...
      // 最佳化交易明細（本次修正：1-1-2 僅三次；先算最大總張數→分配→實際價檢查剩餘資金）
