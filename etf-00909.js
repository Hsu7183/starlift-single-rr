...
(function(){
  ...
  function computeKPIFromOpt(optExecs){
    ...
    for(const e of optExecs){
      ...
      else{ const settle=(e.sellAmount||0)-(e.fee||0)-(e.tax||0); equity+=settle;
    ...
    // 成本級數值在「成本卡」已有明細，這裡不重算
    const totalFees=0,totalTaxes=0,totalCost=0,turnover=0,avgTradeValue=0,costRatio=0;
    ...
    const months=[...byMonth.keys()], winsM=months.filter(m=> byMonth.get(m)>0).length;
  ...
  // =====（新增）每週獲利＋累積淨利圖 =====
  let chWeekly=null;
  function weekStartDate(ms){ // 以週一為週起點（UTC）
    const d=new Date(ms); const day=(d.getUTCDay()+6)%7; // 週一=0
  ...
  function renderWeeklyChart(optExecs){
    ...
    chWeekly=new Chart(ctx,{
      ...
      options:{
        ...
        parsing:{ yAxisKey:undefined }, // 允許 [start,end]
        scales:{
  ...
  function renderKPI(K, bench){
    ...
    if(bench && bench.ok){
      ...
      const tr=document.createElement('tr');
      tr.innerHTML=`<td colspan="4">— ${bench && bench.reason ? bench.reason : '尚未連結基準'}</td>`;
  ...
  function mergeRowsByBaseline(baseRows,newRows){
    ...
    for(const r of newRows){ if(!endBase || r.ts>endBase){ merged.push(r); if(!start8) start8=r.day; end8=r.day; } }
