// etf-00909.js — v7
// 00909專責：KPI置頂 + 基準型KPI + 週次獲利(浮動) + Waterfall + Weekly Heatmap + 週度歸因堆疊 + Daily Heatmap
(function () {
  ...
  async function writeManifest(obj) {
    ...
    await sb.storage.from(CFG.bucket).upload(CFG.manifestPath, blob, { upsert: true, cacheControl: '0', contentType: 'application/json' });
  }
  ...
  function parseTs(ts14) { const Y = +ts14.slice(0, 4), M = +ts14.slice(4, 6) - 1, D = +ts14.slice(6, 8), h = +ts14.slice(8, 10), m = +ts14.slice(10, 12), s = +ts14.slice(12, 14); return new Date(Date.UTC(Y, M, D, h, m, s)).getTime(); }
  function parseCanon(text) {
    ...
    for (const raw of lines) {
      ...
      if (/日期\s*[,|\t]\s*時間\s*[,|\t]\s*價格/i.test(line) || /^日期$|^時間$|^價格$|^動作$|^說明$/i.test(line)) continue;
      let m = line.match(CANON_RE);
      if (m) { rows.push({ ts: m[1], tsMs: parseTs(m[1]), day: m[1].slice(0, 8), price: +m[2], kind: (m[3] === '平賣' ? 'sell' : 'buy'), units: undefined }); continue; }
      const parts = line.split(/[\t,]+/).map(s => s.trim()).filter(Boolean);
      if (parts.length >= 4) {
        const d = parts[0]; if (!/^\d{8}$/.test(d)) continue;
        const t = String(parts[1]).padStart(6, '0'), px = parts[2], zh = parts[3];
        ...
        const ts = d + String(t).padStart(6, '0');
  ...
  function backtest(rows, cfg) {
    ...
    let cumCostFull = 0, unitsInPeriod = 0, cumPnlAll = 0;
    ...
    for (const r of rows) {
      if (r.kind === 'buy') {
        ...
        cash -= cost; cumCostFull += cost; unitsInPeriod += (r.units ?? 1);
        ...
        buyFeeAcc += f.fee;
        execs.push({ side: 'BUY', ts: r.ts, tsMs: r.tsMs, price: r.price, avgCost: newAvg, shares: qty, buyAmount: f.gross, sellAmount: 0, fee: f.fee, tax: 0, cost: cost, cumCost: cumCostFull, pnlFull: null, retPctUnit: null, cumPnlFull: cumPnlAll });
      } else {
        if (shares <= 0) continue;
        const qty = shares, f = fees(r.price, qty, cfg, true);
        const pnlFull = f.gross - cumCostFull - (f.fee + f.tax);
        const retPctUnit = (unitsInPeriod > 0 && cumCostFull > 0) ? (pnlFull / (cumCostFull / unitsInPeriod)) : null;
        cumPnlAll += pnlFull; cash += (f.gross - f.fee - f.tax);
        const costBasis = avgCost * qty;
        const pnlStd = (f.gross - f.fee - f.tax) - costBasis;
        const holdDays = (parseTs(r.ts) - (openTs ? parseTs(openTs) : parseTs(r.ts))) / DAY_MS;
        trades.push({ side: 'LONG', inTs: openTs || r.ts, outTs: r.ts, inPx: openPx || avgCost, outPx: r.price, shares: qty, buyFee: Math.round(buyFeeAcc), sellFee: Math.round(f.fee), sellTax: Math.round(f.tax), pnl: pnlStd, holdDays });
        execs.push({ side: 'SELL', ts: r.ts, tsMs: r.tsMs, price: r.price, avgCost: avgCost, shares: qty, buyAmount: 0, sellAmount: f.gross, fee: f.fee, tax: f.tax, cost: 0, cumCost: cumCostFull, pnlFull, retPctUnit, cumPnlFull: cumPnlAll });
        shares = 0; avgCost = 0; openTs = null; openPx = null; buyFeeAcc = 0; cumCostFull = 0; unitsInPeriod = 0;
      }
  ...
  function _fees(px, sh, isSell) { const gross = px * sh; const fee = Math.max(CFG.minFee, gross * CFG.feeRate); const tax = isSell ? gross * CFG.taxRate : 0; return { gross, fee, tax }; }
  function buyCostLots(px, lots) { const sh = lots * OPT.unitShares; const f = _fees(px, sh, false); return { cost: f.gross + f.fee, shares: sh, f }; }
  function buildOptimizedExecs(execs) {
    ...
    for (const seg of segs) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const unitC = buyCostLots(b.price, 1).cost; let affordable = Math.floor(remaining / unitC); if (affordable <= 0) break;
        ...
        out.push({ side: 'BUY', ts: b.ts, tsMs: b.tsMs, price: b.price, avgCost: newAvg, shares: bc.shares, buyAmount: bc.f.gross, sellAmount: 0, fee: bc.f.fee, tax: 0, cost: bc.cost, cumCost, pnlFull: null, retPctUnit: null, cumPnlFull: cumPnlAll });
      }
      if (sharesHeld > 0) {
        const s = sell; const st = _fees(s.price, sharesHeld, true);
        const pnlFull = st.gross - cumCost - (st.fee + st.tax);
        const retPctUnit = (unitCount > 0 && cumCost > 0) ? (pnlFull / (cumCost / unitCount)) : null;
        cumPnlAll += pnlFull;
        out.push({ side: 'SELL', ts: s.ts, tsMs: s.tsMs, price: s.price, avgCost, shares: sharesHeld, buyAmount: 0, sellAmount: st.gross, fee: st.fee, tax: st.tax, cost: 0, cumCost, pnlFull, retPctUnit, cumPnlFull: cumPnlAll });
      }
    }
    return out;
  }

  // ───── KPI（與前版一致；僅保留必要顯示，避免篇幅過長）
  const BANDS = {
    ...
    PF: { strong: 1.5, adequate: 1.0 }, Hit: { strong: 0.55, adequate: 0.45 }, TU_days: { strong: 45, adequate: 120 },
    Rec_days: { strong: 45, adequate: 90 }, MCL: { strong: 5, adequate: 10 }, Payoff: { strong: 1.5, adequate: 1.0 },
    CostRatio: { strong: 0.001, adequate: 0.003 }, Turnover: { strong: 1.0, adequate: 2.0 }, RtVol: { strong: 1.0, adequate: 0.5 },
    MAR: { strong: 1.0, adequate: 0.3 }, Omega: { strong: 1.5, adequate: 1.0 }, VaR95: { strong: -0.02, adequate: -0.04 },
    CVaR95: { strong: -0.03, adequate: -0.06 }, UI: { strong: 0.05, adequate: 0.12 }, Martin: { strong: 0.8, adequate: 0.3 },
    GPR: { strong: 1.5, adequate: 1.0 }, Tail: { strong: 1.5, adequate: 1.0 },
  };
  ...
  function computeKPIFromOpt(optExecs) {
    ...
    for (const e of optExecs) {
      if (e.side === 'BUY') { equity -= (e.cost || 0); grossBuy += (e.buyAmount || 0); if (segStartMs == null) segStartMs = e.tsMs; }
      else {
        ...
        if (segStartMs != null) { holdDaysArr.push((e.tsMs - segStartMs) / DAY_MS); segStartMs = null; }
      }
    ...
    const skew = (() => { if (rets.length < 3 || sd === 0) return 0; const n = rets.length; return (n / ((n - 1) * (n - 2))) * rets.reduce((s, x) => s + Math.pow((x - mean) / sd, 3), 0); })();
    const kurt = (() => { if (rets.length < 4 || sd === 0) return 0; const n = rets.length; const m4 = rets.reduce((s, x) => s + Math.pow((x - mean), 4), 0) / n; const g2 = m4 / Math.pow(sd, 4) - 3; return g2 + 3; })();
    ...
    for (let i = 1; i < days.length; i++) { const ym = days[i].slice(0, 7); const r = (eqs[i] / eqs[i - 1] - 1); byMonth.set(ym, (byMonth.get(ym) || 0) + r); }
    const months = [...byMonth.keys()]; const winsM = months.filter(m => byMonth.get(m) > 0).length;
  ...
  async function loadBenchmark(days) {
    ...
    try {
      ...
      if (head.includes('ret')) { for (let i = 1; i < lines.length; i++) { const [d, r] = lines[i].split(/[, \t]+/); if (d && !isNaN(+r)) data.push({ d: d.slice(0, 10), r: +r }); } }
        ...
        let prev = null;
        for (let i = 1; i < lines.length; i++) { const [d, c] = lines[i].split(/[, \t]+/); const px = +c; if (d && px > 0) { if (prev != null) data.push({ d: d.slice(0, 10), r: px / prev - 1 }); prev = px; } }
      }
  ...
  function regressXY(x, y) {
    ...
    const beta = sxx > 0 ? sxy / sxx : 0; const alpha = my - beta * mx; const r2 = (sxx > 0 && syy > 0) ? (sxy * sxy) / (sxx * syy) : 0; return { alpha, beta, r2 };
  }
  function benchKPIs(stratRets, benchRets, rf = 0) {
    const n = Math.min(stratRets.length, benchRets.length); if (n < 5) return null;
    ...
    const TE = sdA * Math.sqrt(252), annExcess = meanA * 252, IR = TE > 0 ? annExcess / TE : 0;
  ...
  function pickSuggestions(groups) {
    ...
    return bad.map(x => [x.name, x.value, '—', x.rating, x.band]); // 全列
  }
  ...
  function renderKPI(K, bench) {
    ...
    const cost = [
      ...
      ['MAR (CAGR/MaxDD)', (K.ratios.calmar).toFixed(2), '等同 Calmar', rateHigher(K.ratios.calmar, BANDS.MAR), '≥1.0 / ≥0.3']
    ...
    if (bench && bench.ok) {
      const rows = [
        ...
        ['Upside Capture', bench.upCap == null ? '—' : bench.upCap.toFixed(2), '基準上漲日：策略平均 / 基準平均', '—'],
        ['Downside Capture', bench.dnCap == null ? '—' : bench.dnCap.toFixed(2), '基準下跌日：策略平均 / 基準平均', '—'],
        ['Treynor Ratio', bench.treynor.toFixed(2), '(年化超額)/Beta', '—'],
      ];
      for (const r of rows) { const tr = document.createElement('tr'); tr.innerHTML = `<td>${r[0]}</td><td>${r[1]}</td><td>${r[2]}</td><td>—</td>`; benchBody.appendChild(tr); }
    } else {
      const tr = document.createElement('tr'); tr.innerHTML = `<td colspan="4">— ${bench && bench.reason ? bench.reason : '尚未連結基準'}</td>`; benchBody.appendChild(tr);
    }
  ...
  // ───── 新圖：每週獲利＋累積淨利（浮動長條+折線）
  let chWeekly, chWaterfall, chWeekHeat, chWeekAttr;
  ...
  function buildWeeklyFromOpt(optExecs) {
    const per = new Map(), order = [];
    let feesTaxPerWeek = new Map();
    for (const e of optExecs) {
      if (e.side !== 'SELL') continue;
      const wk = weekStartDate(e.tsMs);
      if (!per.has(wk)) { per.set(wk, 0); order.push(wk); feesTaxPerWeek.set(wk, 0); }
      per.set(wk, per.get(wk) + (e.pnlFull || 0));
      feesTaxPerWeek.set(wk, feesTaxPerWeek.get(wk) + ((e.fee || 0) + (e.tax || 0)));
    }
    const labels = order;
    const weekly = labels.map(wk => per.get(wk) || 0);
    const weeklyCost = labels.map(wk => -(feesTaxPerWeek.get(wk) || 0)); // 成本視為負貢獻
    const cum = []; let s = 0; for (const v of weekly) { s += v; cum.push(s); }
    return { labels, weekly, weeklyCost, cum };
  }
  function mkChartLine(ctx, labels, data, title) {
    return new Chart(ctx, { type: 'line', data: { labels, datasets: [{ label: title, data, borderWidth: 2, tension: 0.2, pointRadius: 0 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { ticks: { maxTicksLimit: 14 } } } } });
  }
  function renderWeeklyChart(optExecs) {
    const box = $('#insightsCard'); box.style.display = '';
    // 週次圖
    ...
    chWeekly = new Chart(ctx, {
      data: {
        labels: W.labels,
        datasets: [
          { type: 'bar', label: '每週獲利（浮動）', data: floatBars, borderWidth: 1 },
      ...
      options: {
        ...
        scales: { y: { suggestedMin: 0, suggestedMax: Math.max(1, Math.max(...W.cum) * 1.05) }, x: { ticks: { maxTicksLimit: 14 } } }
      }
    });

    // Waterfall：單純顯示每週變化量，浮動接續
    const wf = $('#chWaterfall').getContext('2d');
    const wfBars = []; let cur = 0;
    for (const v of W.weekly) { const next = cur + v; wfBars.push([cur, next]); cur = next; }
    if (chWaterfall) chWaterfall.destroy();
    chWaterfall = new Chart(wf, {
      type: 'bar',
      data: { labels: W.labels, datasets: [{ label: 'Waterfall（週變化）', data: wfBars }] },
      options: { responsive: true, maintainAspectRatio: false, parsing: { yAxisKey: undefined }, plugins: { legend: { display: false } }, scales: { x: { ticks: { maxTicksLimit: 14 } } } }
    });

    // 週度歸因堆疊：淨利 + （賣方）手續費稅（成本）
    const attr = $('#chWeekAttr').getContext('2d');
    if (chWeekAttr) chWeekAttr.destroy();
    chWeekAttr = new Chart(attr, {
      type: 'bar',
      data: {
        labels: W.labels,
        datasets: [
          { label: '淨利（實現）', data: W.weekly, stack: 's' },
          { label: '成本（賣方費用+稅）', data: W.weeklyCost, stack: 's' },
        ]
      },
      options: {
        responsive: true, maintainAspectRatio: false, plugins: { legend: { display: true } },
        scales: { x: { stacked: true, ticks: { maxTicksLimit: 14 } }, y: { stacked: true } }
      }
    });

    // Weekly Heatmap（原生 Canvas）
    drawWeekHeat($('#chWeekHeat'), W.labels, W.weekly);
  }

  // ───── Heatmap（原生 Canvas）
  function colorScaleRG(v, min, max) {
    if (max === min) return '#cccccc';
    const t = Math.max(-1, Math.min(1, (v - 0) / (Math.max(Math.abs(min), Math.abs(max))))); // -1..1
    // 綠(負)→白→紅(正)
    if (t >= 0) { const r = 255, g = Math.round(255 * (1 - t)), b = Math.round(255 * (1 - t)); return `rgb(${r},${g},${b})`; }
    const tt = Math.abs(t); const r = Math.round(255 * (1 - tt)), g = 255, b = Math.round(255 * (1 - tt)); return `rgb(${r},${g},${b})`;
  }
  function drawWeekHeat(canvas, labels, weekly) {
    ...
    const maxAbs = Math.max(...weekly.map(x => Math.abs(x))) || 1;
    const pad = 40, cellW = Math.max(6, Math.floor((W - pad) / n)), cellH = Math.max(18, H - 28);
    ctx.clearRect(0, 0, W, H);
    // cells
    for (let i = 0; i < n; i++) {
      const x = pad + i * cellW, y = 10;
      ctx.fillStyle = colorScaleRG(weekly[i], -maxAbs, maxAbs);
      ctx.fillRect(x, y, cellW - 2, cellH);
    }
    // axis labels (approx every 6)
    ctx.fillStyle = '#666'; ctx.font = '12px sans-serif';
    const step = Math.ceil(n / 10);
    for (let i = 0; i < n; i += step) { ctx.fillText(labels[i].slice(5), pad + i * cellW, H - 4); }
    ctx.fillText('週次熱力（紅=賺、綠=虧）', 6, 14);
  }
  function drawDailyHeat(canvas, days, rets) {
    const ctx = canvas.getContext('2d'); const W = canvas.width = canvas.clientWidth; const H = canvas.height = canvas.clientHeight;
    if (days.length < 2) return;
    // row=月份, col=日 1..31
    const byMonth = new Map();
    for (let i = 0; i < days.length; i++) {
      const d = days[i]; const m = d.slice(0, 7); const day = +d.slice(8, 10);
      if (!byMonth.has(m)) byMonth.set(m, new Map());
      byMonth.get(m).set(day, rets[i - 1] || 0); // 第一天無報酬，略過
    }
    const months = [...byMonth.keys()];
    const rows = months.length, cols = 31;
    const padL = 60, padT = 20, cellW = Math.floor((W - padL - 10) / cols), cellH = Math.floor((H - padT - 10) / Math.max(1, rows));
    const allVals = []; byMonth.forEach(map => map.forEach(v => allVals.push(v)));
    const maxAbs = Math.max(0.001, ...allVals.map(x => Math.abs(x)));
    ctx.clearRect(0, 0, W, H);
    ctx.font = '12px sans-serif'; ctx.fillStyle = '#666';
    ctx.fillText('日報酬 Heatmap（行=月份，列=日期）', 6, 14);
    for (let r = 0; r < rows; r++) {
      const m = months[r]; const map = byMonth.get(m);
      ctx.fillStyle = '#444'; ctx.fillText(m, 8, padT + r * cellH + Math.min(cellH - 4, 14));
      for (let c = 1; c <= cols; c++) {
        const v = map.get(c);
        const x = padL + (c - 1) * cellW, y = padT + r * cellH;
        ctx.fillStyle = v == null ? '#eee' : colorScaleRG(v, -maxAbs, maxAbs);
  ...
  async function boot() {
    try {
      ...
      const rowsNew = parseCanon(txtNew);
      ...
      if (base) {
        ...
        const rowsBase = parseCanon(await fetchText(baseUrl));
        const endBase = rowsBase.length ? rowsBase[rowsBase.length - 1].ts : null;
        const merged = [...rowsBase]; for (const r of rowsNew) { if (!endBase || r.ts > endBase) { merged.push(r); if (!start8) start8 = r.day; end8 = r.day; } }
        rowsMerged = merged;
      } else { start8 = rowsNew[0].day; end8 = rowsNew.at(-1).day; }
      $('#periodText').textContent = `期間：${start8 || '—'} 開始到 ${end8 || '—'} 結束`;

      // 回測 + 明細
      const bt = backtest(rowsMerged, CFG);
      ...
      // Daily Heatmap 以日報酬 rets 繪製
      drawDailyHeat($('#chDailyHeat'), K.equity.days, K.returns.daily);
